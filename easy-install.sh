...
echo -n "SatNOGS Station PPM: "
read SATNOGS_PPM_ERROR
echo "[satnogs]
localhost SATNOGS_API_URL=$SATNOGS_API_URL SATNOGS_API_TOKEN=$SATNOGS_API_TOKEN SATNOGS_STATION_ID=$SATNOGS_STATION_ID SATNOGS_PPM_ERROR=$SATNOGS_PPM_ERROR SATNOGS_STATION_LAT=$SATNOGS_STATION_LAT SATNOGS_STATION_LON=$SATNOGS_STATION_LON SATNOGS_STATION_ELEV=$SATNOGS_STATION_ELEV
" > hosts
echo "- hosts: satnogs
  serial: 1
  become: yes
